...
local function Improved_Precaching_Weap()
     print("---------------")

     local Impr_CountPrecache = 0
     for class, tbl in ipairs(weapons.GetList()) do
          Impr_Count_Delay  = Impr_Count_Delay + Improved_Caching_Sys
          local ipr_model_weap = tbl.WorldModel

          if (ipr_model_weap and ipr_model_weap ~= "") then
               timer.Simple(Impr_Count_Delay, function()
               Impr_CountPrecache  = Impr_CountPrecache + 1
               util.PrecacheModel(ipr_model_weap)
               print(ipr_model_weap, "Caching : " ..Impr_CountPrecache)
               end)
          end
     end
